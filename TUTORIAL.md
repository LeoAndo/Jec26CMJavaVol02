...
教材に含まれているコンパイルエラー例は、単に「間違い」ではなく、Java の文法上どこが許されないかを確認するためのものです。

**注意**

- コンパイラのエラーメッセージは、JDK のバージョンによって表現が少し変わることがあります。
- エラーメッセージは原因をそのまま説明するとは限らず、構文解析の途中で検出された結果として表示されることがあります。

| ファイル | 誤った記述 | 学ぶポイント | 代表的なエラーメッセージ |
|---------|-----------|-------------|--------------------------|
| `Test0206.java` | `System.out.println(018);` | 8進数リテラルには `8` を使えない | `')'または','がありません` |
| `Test0207.java` | `System.out.println(123.4L);` | `L` は整数リテラル用であり、小数には付けられない | `')'または','がありません` / `';'がありません` |
| `Test0210.java` | `System.out.println('AB');` | `char` リテラルは 1 文字しか書けない | `文字リテラルが閉じられていません` / `文ではありません` |
| `Test0214.java` | `System.out.print("AB"C");` | 文字列中で `"` を使うときは `\\\"` と書く必要がある | `')'または','がありません` / `';'がありません` / `文ではありません` |

それぞれのファイルを実際にコンパイルして、どの位置でエラーが出るかを確認すると理解しやすくなります。
