...
通常のサンプルは、IntelliJ IDEA で開いたファイルの `main` メソッドを実行するだけで確認できます。
`Test0206.java`、`Test0207.java`、`Test0210.java`、`Test0214.java` は、実行ではなくコンパイルエラーを確認するための教材です。
...
一部の教材ファイルには、学習用として **意図的にコンパイルエラーになる記述** が含まれています。
そのため、このリポジトリは `javac src/*.java` による一括コンパイルを前提にしていません。
必要なファイルを 1 本ずつ確認してください。
